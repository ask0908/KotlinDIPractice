...
//        Log.e("CoinViewModel", "query : $query")
    ...
    val testUpbitLive: LiveData<CoinData> = requestCoinLive.switchMap { query ->
        liveData(viewModelScope.coroutineContext + Dispatchers.IO) {
            CoinRepo.webService.loadPriceTest("CRIX.UPBIT.KRW-BTC").asCallbackFLow().catch { e ->
                Log.e("CoinViewModel", "에러 : $e")
            }.collect {
                this.emit(it)
            }
        }
